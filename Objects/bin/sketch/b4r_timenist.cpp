...
b4r_timenist::_main->_timeisavailable();
...
B4R::StackMemory::cp = cp;
if (true) return B4R::StackMemory::ReturnArrayOnStack(b4r_timenist::_date, sizeof(Byte));
...
B4R::StackMemory::cp = cp;
if (true) return (UInt) (Common_Floor(_getupdatedtotalseconds()/(Double)3600));
...
B4R::StackMemory::cp = cp;
if (true) return (UInt) (_minutes%60);
...
B4R::StackMemory::cp = cp;
if (true) return (UInt) (_getupdatedtotalseconds()%60);
...
B4R::StackMemory::cp = cp;
if (true) return (ULong) (b4r_timenist::_seconds+(Common_Millis()-b4r_timenist::_lastmillis)/(Double)1000);
